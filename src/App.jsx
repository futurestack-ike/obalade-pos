...
const DEFAULT_MENU = [
  ...
  { name: "AppleSap", category: "Drinks", price: 4.0, available: true },
  { name: "B.Lemmon", category: "Drinks", price: 4.0, available: true },
  { name: "OrangeSap", category: "Drinks", price: 4.0, available: true },
  { name: "Malt", category: "Drinks", price: 4.0, available: true },
  { name: "Red Bull", category: "Drinks", price: 4.0, available: true },
  { name: "Fernandes", category: "Drinks", price: 4.0, available: true },
  { name: "Ginger Beer", category: "Drinks", price: 4.0, available: true },
  { name: "Star / Gulder / Club Beer", category: "Drinks", price: 8.0, available: true },
  { name: "Palm Wine", category: "Drinks", price: 10.0, available: true },
  ...
  { name: "Jagermeister (shot)", category: "Drinks", price: 5.0, available: true },
  { name: "Red Label (shot)", category: "Drinks", price: 5.0, available: true },
  { name: "Black Label (shot)", category: "Drinks", price: 5.0, available: true },
  { name: "Campari (shot)", category: "Drinks", price: 5.0, available: true },
  { name: "Baileys (shot)", category: "Drinks", price: 5.0, available: true },
  { name: "Safari (shot)", category: "Drinks", price: 5.0, available: true },
  { name: "Disaronno (shot)", category: "Drinks", price: 5.0, available: true },
  { name: "Hennessy (shot)", category: "Drinks", price: 5.0, available: true },
];
...
const PinScreen = ({title,onSuccess,onCancel}) => {
  ...
  return (
    <div className="pg"><div className="pinwrap">
      <div style={{textAlign:"center"}}><div className="sh">{title}</div><div style={{color:"var(--muted)",fontSize:13,marginTop:4}}>Enter {pinLen}-digit PIN</div></div>
...
// ── New Order ─────────────────────────────────────────────────────
const NewOrderScreen = ({onStart, onAddToExisting, activeOrders}) => {
  const [type,setType]=useState(null); const [ref,setRef]=useState("");
  const [dupOrder,setDupOrder]=useState(null);

  const handleContinue = () => {
    if(type==="dine-in" && ref.trim()) {
      // Check for existing open orders on same table
      const existing = activeOrders.filter(o=>
        o.type==="dine-in" &&
        o.table_number===ref.trim() &&
        o.status!=="paid"
      );
      if(existing.length>0) { setDupOrder(existing[0]); return; }
    }
    if(type==="takeaway" && ref.trim()) {
      const existing = activeOrders.filter(o=>
        o.type==="takeaway" &&
        o.table_number===ref.trim() &&
        o.status!=="paid"
  ...
  return (
    <div className="pg">
      ...
      <div className="tgrid">
        <div className={`ttile${type==="dine-in"?" on":""}`} onClick={()=>{setType("dine-in");setDupOrder(null);}}><Ic.DineIn/> Dine In</div>
        <div className={`ttile${type==="takeaway"?" on":""}`} onClick={()=>{setType("takeaway");setDupOrder(null);}}><Ic.Takeaway/> Takeaway</div>
      </div>
      {type&&<><label className="lbl">{type==="dine-in"?"Table number *":"Customer name (optional)"}</label>
        <input className="inp" placeholder={type==="dine-in"?"e.g. 4":"e.g. Ahmed"} value={ref} onChange={e=>{setRef(e.target.value);setDupOrder(null);}} autoFocus/></>}

      {dupOrder&&(
        <div style={{background:"#fff8e6",border:"1.5px solid var(--gold)",borderRadius:12,padding:16,marginBottom:16}}>
          <div style={{fontWeight:700,fontSize:14,color:"var(--gold)",marginBottom:8}}>
            ⚠️ {type==="dine-in"?`Table ${ref}`:`${ref}`} already has an open order
          </div>
          <div style={{fontSize:13,color:"var(--muted)",marginBottom:12}}>
            Do you want to add items to the existing order, or create a new separate order?
          </div>
          <div style={{display:"flex",gap:8}}>
            <button className="btn bp bsm" style={{flex:1}} onClick={()=>onAddToExisting(dupOrder)}>+ Add to existing</button>
            <button className="btn bo bsm" style={{flex:1}} onClick={()=>onStart(type,ref.trim())}>Create new order</button>
          </div>
        </div>
      )}

      {!dupOrder&&<button className="btn bp bw" disabled={!type||(type==="dine-in"&&!ref.trim())} onClick={handleContinue}>Continue to Menu →</button>}
    </div>
  );
};
...
const vibrateDevice = (pattern=[180,80,180,80,400]) => {
...
// Ready sound — warm double-chime: DONG-dong
// Positive completion feel, clearly different from kitchen alert
const playReadySound = () => {
  try {
    ...
    const chime = (freq, t, vol, decay) => {
      // Warm bell using sine + gentle harmonics
    ...
    // DONG (high, loud) then dong (lower, softer) — order is ready!
    chime(1318, 0.00, 0.65, 1.2);  // E6 — main chime
    chime(1047, 0.28, 0.50, 1.0);  // C6 — resolution
    // Short single buzz — gentler than kitchen alert
    vibrateDevice([300]);
  } catch(e) { console.error("Ready sound error:", e); }
};
    ...
    <div className={wrap}>
      {desktop
        ? <div><div className="desktop-panel-title">Orders</div><div className="desktop-panel-sub">Tap to view details & payment</div>
            ...
            {soundOn&&<div style={{background:"var(--gp)",border:"1px solid var(--g)",borderRadius:10,padding:"8px 14px",fontSize:12,color:"var(--g)",fontWeight:600,marginBottom:16,cursor:"pointer"}} onClick={playReadySound}>
        ...
        : <><div className="row" style={{marginBottom:4}}>
          ...
          {soundOn&&<div style={{background:"var(--gp)",border:"1px solid var(--g)",borderRadius:10,padding:"8px 14px",fontSize:12,color:"var(--g)",fontWeight:600,marginBottom:12,cursor:"pointer"}} onClick={playReadySound}>
            🔔 Sound active — tap to test · alerts when order is ready
          </div>}
      ...
      <div className="action-bar">
        ...
        {order.status!=="paid"&&(
          <>
            ...
            {order.status!=="ready"&&<button className="btn bo bsm" style={{flex:1}} onClick={()=>onEdit(order)}><Ic.Edit/> Edit</button>}
          </>
        )}
        {isAdmin&&order.status==="paid"&&<button className="btn bd bsm" onClick={()=>setConfirmDel(true)}><Ic.Trash/> Delete</button>}
        {order.status!=="paid"&&<button className="btn bd bsm" onClick={()=>setConfirmDel(true)}><Ic.Trash/> Delete</button>}
      </div>
      ...
      newOrders.filter(ord=>ord.status==="ready").forEach(ord=>{
        if(!prevReadyIds.current.has(ord.id)){
          ...
          if(staffSoundMode.current){
            if(audioCtx && audioCtx.state==="suspended"){
              audioCtx.resume().then(()=>playReadySound());
            } else {
              playReadySound();
            }
          }
